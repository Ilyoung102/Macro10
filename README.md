...
## 주의사항
- **자동 클릭 기능 제한**: 안드로이드 보안 정책상, 일반 앱이 다른 앱(예: CGV 앱) 화면을 마음대로 클릭할 수 없습니다. 따라서 이 앱은 **"시간이 되면 사이트를 열고, 화면에 완료 표시를 띄우는"** 기능으로 변경되었습니다.
- **사이트 자동 열기**: 설정한 시간이 되면 스마트폰의 기본 브라우저로 해당 사이트가 열립니다.
# Macro10
